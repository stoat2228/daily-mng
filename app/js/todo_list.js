...
function AddToDoTitle() {
    ...
    */
    ...
    //リクエスト
    let request = {
        newTitle: encodeURI(document.getElementById("newTitle").value)
    };
    //ajax
    $.ajax({
        url: '/cgi/ToDoTitle/addToDoTitle.py',
        type: "POST",
...
function ToDoListOnLoad() {
    ...
    //ajax
    $.ajax({
        url: '/cgi/ToDoTitle/getToDoTitle.py',
        type: "POST",
        //data: JSON.stringify(request),  //object -> json
        async: true,                    //true:非同期(デフォルト), false:同期
        dataType: "json",
        success: function (data) {
            response = data;
            document.getElementById("title").innerHTML = response.titleList;
...
function titleChange() {
    ...
    //リクエスト
    let request = {
        incomplete_flg: document.getElementById("incomplete").checked,
        target_value: document.getElementById("title").value
    };
    //ajax
    $.ajax({
        url: '/cgi/ToDoList/getToDoList.py',
        type: "POST",
        data: JSON.stringify(request),  //object -> json
        async: true,                    //true:非同期(デフォルト), false:同期
        dataType: "json",
        success: function (data) {
            ...
            document.getElementById("todo_list").innerHTML = response.toDoList;
...
function sortableSet(obj) {
    ...
    if (obj.value == "並べ替え") {
        ...
        return;
        var objTBL = document.getElementById("toDoListTable");
        if (!objTBL)
            return;
        ...
        for (var i = 1; i < objTBL.rows.length; i++) {
            ...
            //ajax
            $.ajax({
                url: '/cgi/ToDoList/sortToDoList.py',
                type: "POST",
                data: JSON.stringify(request),  //object -> json
                async: true,                    //true:非同期(デフォルト), false:同期
                dataType: "json",
...
function deleteRow(obj) {
    ...
    if (document.getElementById("Id" + rowId).value) {
        ...
        //ajax
        $.ajax({
            url: '/cgi/ToDoList/deleteToDoList.py',
            type: "POST",
            data: JSON.stringify(request),  //object -> json
            async: true,                    //true:非同期(デフォルト), false:同期
            dataType: "json",
...
function appendRow() {
    ...
    c2.innerHTML = '<input type="checkbox" class="check" id="check' + String(count) + '" onchange="dataEdit(this)">'
    c3.innerHTML = '<input type="text" class="content"  size="40" maxlength="30" id="content' + String(count) + '" value="" onchange="dataEdit(this)"></input>'
    c4.innerHTML = '<input type="text" class="editFlg" size="3" maxlength="2" id="editFlg' + String(count) + '" value="0">'
    c5.innerHTML = '<input type="text" class="Id" size="3" maxlength="2" id="Id' + String(count) + '" value="">'
    ...
    for (var i = 1; i < objTBL.rows.length; i++) {
        ...
        if (updateFlg.value == 1) {
            ...
            let request = {
                ...
                title_id: encodeURI(document.getElementById("title").value),
                content: encodeURI(document.getElementById("content" + i).value),
                ItemId: encodeURI(document.getElementById("Id" + i).value)
            };
            //ajax
            $.ajax({
                url: '/cgi/ToDoList/registToDoList.py',
                type: "POST",
                data: JSON.stringify(request),  //object -> json
                async: true,                    //true:非同期(デフォルト), false:同期
                dataType: "json",
